function editable(){ //將ox原先提供的惟讀模式改成可編輯模式
    var ids = ['q24', 'q26', 'q28', 'q30', 'q32', 'qsum', 'qware', 'qshelf', 'qx', 'qy', 'speed', 's24', 's26', 's28', 's30', 's32', 'ssum2', 'sware', 'sshelf', 'sx', 'sy', 'memo', 'status'];
    var szs = ['3', '3', '3', '3', '3', '6', '8', '6', '3', '3', '6', '3', '3', '3', '3', '3', '6', '8', '6', '3', '3', '12', '0']; 
...
alert('placeDetail.js: oid_txt: '+ oid_txt);
        ...
        $(this).find('td:gt(4)').each(//訂單的尺寸數量(惟讀)
            function(i){
         	    var data = $.trim(this.childNodes[0].data);
                if(!data){ data = ''; }
  	             //$(this).find(':first-child').remove();
  	             this.removeChild(this.childNodes[0]);//移除原來的label資料
      	       $(this).append('<input id="'+ ids[i] +'_'+ sn + '" '+ types[i] +' value="' + data +'" size="'+ szs[i] +'" readonly style="border:0;background:#'+ bgcolor +' none repeat scroll 0 0;">');
                }
           );
//        $(this).find('td').slice(16,28).each(//本上架單的尺寸數量(可編輯)
//            function(i){
// 	             this.removeChild(this.childNodes[0]);//移除原來的label資料
//        	       $(this).append('<input id="'+ ids[i] +'_'+ sn + '" '+ types[i] +' size="'+ szs[i] +'"/>');
//                }//
//           );

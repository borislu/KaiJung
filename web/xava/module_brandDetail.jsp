...
<head>
	...
	<link href="<%=request.getContextPath()%>/xava/style/liferay51/css/zhipin.css" rel="stylesheet" type="text/css"> 
	<% 
...
<% } %>
  ...
  function afterSave(){
    ...
    parent.frames["frameEast"].openxava.executeAction('KaiJung', 'BrandListOnly', '', false, 'List.goPage', 'page=1');
    parent.frames["frameEast"].window.location.reload();
  }
			function afterDel(){
						 //alert("trying: "+ $("#ox_KaiJung_BrandDetailOnly__messages_table").length>0);
						 if ($("#ox_KaiJung_BrandDetailOnly__messages_table").length>0) {
						     parent.frames["frameEast"].window.location.reload();
         $.cookie("JSESSIONID", null);
						 }else {
  ...
  function changeLink(){  
    if( ($('a')==null) || ($('a').length < 1) ){
      ...
      //alert( $("a[href^='javascript:openxava.executeAction(\'KaiJung\', \'BrandListOnly\', \'\', false, \'List.viewDetail\'']") );
      //alert( 'a: '+ $("a[href^='javascript:openxava.executeAction(\'KaiJung\', \'BrandListOnly\', \'\', false, \'List.viewDetail\'']").length );
      //$("a[href^='javascript:openxava.executeAction(\'KaiJung\', \'BrandListOnly\', \'\', false, \'List.viewDetail\'']").each(function(index){this.href="javascript:parent.frames['frameWest'].openxava.executeAction('KaiJung', 'BrandDetailOnly', '', false, 'List.viewDetail', 'row="+ index +"')" });
      $('#ox_KaiJung_BrandDetailOnly__CRUD___save').attr('onclick','javascript:afterSave();');
      $('#ox_KaiJung_BrandDetailOnly__CRUD___delete').attr('onclick','javascript:afterDel();');
    }
  }
  changeLink();
	</script>
